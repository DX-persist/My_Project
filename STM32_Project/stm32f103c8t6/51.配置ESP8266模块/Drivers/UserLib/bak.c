...
static void BSP_ESP9266_GPIO_Init(void)
{
...
static void BSP_ESP8266_Mode_Select(bsp_esp8266_t id, bsp_esp8266_mode_t mode)
{
	...
	if(mode == BSP_ESP8266_Flash_Mode){
		GPIO_WriteBit(hw->gpio_port, hw->gpio_pin, Bit_RESET);
	}else{
		GPIO_WriteBit(hw->gpio_port, hw->gpio_pin, Bit_SET);
	}
}

static void BSP_ESP8266_ENABLE(bsp_esp8266_t id)
{
	if(id >= BSP_ESP8266_MAX)	return;
...
static void BSP_ESP8266_GPIO2_ENABLE(bsp_esp8266_t id)
{
	if(id >= BSP_ESP8266_MAX)	return;
...
static void BSP_ESP8266_HardReset(bsp_esp8266_t id)
{
	if(id >= BSP_ESP8266_MAX)	return;
...
{
	BSP_ESP9266_GPIO_Init();													/*配置 GPIO 的端口、引脚、模式、速度、启用 RCC 时钟*/
	BSP_ESP8266_ENABLE(BSP_ESP8266_EN);											/* 使能 ESP8266 模块*/
	BSP_ESP8266_Mode_Select(BSP_ESP8266_GPIO0, BSP_ESP8266_Normal_Mode);		/*配置 ESP8266 模块正常启动 */
	BSP_ESP8266_GPIO2_ENABLE(BSP_ESP8266_GPIO2);								/* 启动配置引脚 */
	BSP_ESP8266_HardReset(BSP_ESP8266_RST);										/* 复位一下 ESP8266 模块*/
}
