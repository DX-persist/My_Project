...
int main(void)
{
  ...
  BSP_ESP8266_SendCmd((uint8_t *)"09.使用WIFI模块控制LED\r\n");
  ...
  while(1){
    ...
    if(bsp_esp8266_rx_done){
      ...
      if(strstr((const char *)bsp_esp8266_rx_buf, BSP_ESP8266_LED_GREEN_ON) != NULL){
        BSP_LED_On(LED_GREEN);
        BSP_LED_Off(LED_BLUE);
        BSP_LED_Off(LED_RED);
      }else if(strstr((const char *)bsp_esp8266_rx_buf, BSP_ESP8266_LED_BLUE_ON) != NULL){
        BSP_LED_On(LED_BLUE);
        BSP_LED_Off(LED_GREEN);
        BSP_LED_Off(LED_RED);
      }else if(strstr((const char *)bsp_esp8266_rx_buf, BSP_ESP8266_LED_RED_ON) != NULL){
        BSP_LED_On(LED_RED);
        BSP_LED_Off(LED_GREEN);
        BSP_LED_Off(LED_BLUE);
      }else if(strstr((const char *)bsp_esp8266_rx_buf, BSP_ESP8266_LED_ALL_OFF) != NULL){
        BSP_LED_Off(LED_GREEN);
        BSP_LED_Off(LED_BLUE);
        BSP_LED_Off(LED_RED);
      }
    ...
    // BSP_ESP8266_SendCmd((uint8_t *)"hello,server\r\n");
    // BSP_Delay_ms(1000);
  }
}