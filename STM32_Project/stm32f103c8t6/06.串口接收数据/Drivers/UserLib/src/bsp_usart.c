...
volatile uint16_t recv_buf_len;
volatile uint8_t recv_buf[RECV_BUF_SIZE] = {0};
...
int _read(int fd, char *ptr, int len)
{
	...
	/* 关闭RXNE中断防止接收到数据后在中断数据会被读走导致scanf函数阻塞 */
	BSP_USART_Set_ITRXNE_State(g_usart_id, DISABLE);
	
	for(int i = 0; i < len; i++){
		/* 判断接受数据寄存器里边是否有数据 */
		while(USART_GetFlagStatus(bsp_usart_hw[g_usart_id].usart, USART_FLAG_RXNE) == RESET);
		*(ptr+i) = (char)USART_ReceiveData(bsp_usart_hw[g_usart_id].usart);
		if(*(ptr+i) == '\n'|| *(ptr+i) == '\r'){
		}
	}
...
void BSP_USART_InputByte(uint8_t recv_byte)
{
	/*
	 ...
	 *	被置为1后，就将数据重新发送到串口
	 */
	if(recv_buf_len < RECV_BUF_SIZE - 1){
		if(recv_byte != '\n'){
