...
static void BSP_USART_GPIO_Init(bsp_usart_t id)
{
	...
	/* 开启 USARTx 的 TX 所在端口的时钟 */
	RCC_APB2PeriphClockCmd(hw->tx_clk, ENABLE);
	
	/* 开启 USARTx 的 RX 所在端口的时钟 */
	RCC_APB2PeriphClockCmd(hw->rx_clk, ENABLE);
