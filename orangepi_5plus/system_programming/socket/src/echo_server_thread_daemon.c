...
int get_hostaddress(char *cmd, socket_config_t *server)
{
	...
	if(fgets(server->ip, sizeof(server->ip), fp) == NULL){
		syslog(LOG_ERR, "fgets error: %s",strerror(errno));
		pclose(fp);
		return -1;
	}
	...
	if(strlen(server->ip) == 0){
		syslog(LOG_ERR, "Failed to get vaild IP address");
		return -1;
	}
...
int listen_func(socket_config_t *server)
{
	...
	syslog(LOG_DEBUG, "Host IP:[%s] Port:[%d] is waiting for connection...",
...
void out_client_msg(socket_config_t *client)
{
	...
	syslog(LOG_DEBUG, "client [%s](%d) connected...",ip, port);
}
...
void commun_func(int new_fd)
{
	...
	//size > 0表示正常读取到了数据
		...
		syslog(LOG_DEBUG, "Server receive TID:[0x%lx] client's message is : %s",
