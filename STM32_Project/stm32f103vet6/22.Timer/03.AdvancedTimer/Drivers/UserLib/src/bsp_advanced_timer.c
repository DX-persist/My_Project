 ...
 * @brief   STM32F10x 高级定时器基础时基/更新中断 BSP 实现文件
 * @details
 * 本文件实现 TIM1 / TIM8 的基础时基初始化与更新中断处理功能。
 *
 * 虽然 TIM1 / TIM8 属于高级定时器，但当前实现只使用其以下能力：
 * - 基础时基计数
 * - 更新事件
 * - 更新中断
 *
 ...
 *
 * @note
 * 本文件并未使用高级定时器的输出比较、PWM、互补输出、BDTR 等功能。
 */
 ...
 */
static uint16_t BSP_TIM_ConvertCounterMode(bsp_advancedtimer_counter_mode_t counter_mode)
{
    switch(counter_mode){
 ...
 */
static uint16_t BSP_TIM_ConvertClockDivision(bsp_advancedtimer_clock_div_t clock_div)
{
    switch(clock_div){
 ...
 */
void BSP_BaseTIM_Init(bsp_advancedtimer_t timer_id, bsp_advancedtimer_config_t *config)
{
 ...
 * @details
 * 当前仅处理更新中断：
 * - 更新计数数组 `advancedtimer_cnt[timer_id]`
 * - 清除中断挂起位
 *
 * @param[in] timer_id 高级定时器编号
 * @return 无返回值
