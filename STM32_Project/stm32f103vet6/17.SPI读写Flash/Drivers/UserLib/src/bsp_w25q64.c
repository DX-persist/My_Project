...
static uint8_t BSP_W25Q64_ReadRegister1(void)
{
	...
	/* 发送伪指令接收从机返回的寄存器状态 */
	BSP_W25Q64_ReadWriteByte(W25Q64_DUMMY_BYTE, &reg_status);
...
uint32_t BSP_W25Q64_ReadID(void)
{
	 ...
	 * 个字节的数据
	 * */
	BSP_W25Q64_ReadWriteByte(W25Q64_DUMMY_BYTE, &temp_byte);
 ...
 * @retval 无
 */
void BSP_W25Q64_ReadData(uint8_t *read_buffer, uint16_t size, uint32_t read_addr)
{
	W25Q64_CS_ENABLE();
	...
	for(int i = 0; i < size; i++){
		BSP_W25Q64_ReadWriteByte(W25Q64_DUMMY_BYTE, &read_buffer[i]);
	}
...
void BSP_W25Q64_Test(void)
{
	...
	BSP_W25Q64_ReadData(read_buffer, len, W25Q64_READ_ADDR);
	for(int i = 0; i < len; i++){
	...
	*/
	...
	BSP_W25Q64_ReadData((uint8_t *)read_buffer2, len * 2, W25Q64_READ_ADDR);
	for(int i = 0; i < len; i++){
	...
	BSP_W25Q64_ReadData((uint8_t *)&read_float_data, sizeof(float), W25Q64_READ_ADDR);
	...
	BSP_W25Q64_ReadData((uint8_t *)read_char_data, strlen(write_char_data), W25Q64_READ_ADDR);
