...
#define PROID			"M4G6ztRO3H"

#define ACCESS_KEY		"VmVXQjJaSzIweHdGdW1XcU5FU2ZVTnNPNHZWaWhNeWE="
...
extern DHT22_Data_TypeDef Data;
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	//sprintf(text, "\"Temp\":{\"value\":%.1f},", Data.temperature);
    sprintf(text, "\"Temp\":{\"value\":%.1f},", Data.temperature);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	//sprintf(text, "\"Humi\":{\"value\":%.1f}", Data.humidity);
    sprintf(text, "\"Humi\":{\"value\":%.1f},", Data.humidity);
	strcat(buf, text);
		...
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(data_ptr)
				{
					...
					OneNET_Publish(topic_buf, "receive,over...");										//回复命令
				}
