 ...
 * @note 基于I2C接口实现EEPROM的字节写入/读取、页写入、顺序读取等功能
 *       支持写周期轮询检测,确保数据可靠写入
 */
 ...
 */
void BSP_EEPROM_WriteByte(uint8_t write_addr, uint8_t write_data)
{
	i2c_status_t status;
 ...
 *       - 写入后需等待约5ms的内部写周期
 */
void BSP_EEPROM_WritePage(uint8_t write_addr, uint8_t *buffer, uint8_t size)
{
 ...
 */
void BSP_EEPROM_ReadRandom(uint8_t read_addr, uint8_t *read_data)
{
	i2c_status_t status;
 ...
 */
void BSP_EEPROM_ReadSequential(uint8_t read_addr, uint8_t *buffer, uint8_t size)
{
	i2c_status_t status;
