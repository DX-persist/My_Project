...
void BSP_USART_ClearRxBuffer(bsp_usart_t id)
{
	if(id >= BSP_USART_MAX)	return;
...
void BSP_USART_CommandHandler(bsp_usart_t id)
{
	...
	// 在判断 RXNE 之前，先检查并清除溢出错误
    if (USART_GetFlagStatus(hw->usart, USART_FLAG_ORE) != RESET) {
        USART_ReceiveData(hw->usart); // 通过读 DR 寄存器来清除 ORE 标志
    }
