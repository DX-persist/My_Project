...
static const bsp_usart_hw_t bsp_usart_hw[BSP_USART_MAX] = {
    [BSP_USART1] = {
        ...
        .irq_it             = USART_IT_RXNE,            /* 判断是否在串口1的 RXNE 标志位上产生了中断 */
    ...
    [BSP_USART2] = {
        ...
        .irq_it             = USART_IT_RXNE,            /* 判断是否在串口2的 RXNE 标志位上产生了中断 */
        .irq_preempt_prio   = BSP_USART_PREEMPT_PRIO,   /* 抢占优先级 */
        .irq_sub_prio       = BSP_USART_SUB_PRIO,       /* 响应优先级 */
    },
    [BSP_USART3] = {
        ...
        .irq_it             = USART_IT_RXNE,            /* 判断是否在串口3的 RXNE 标志位上产生了中断 */
        .irq_preempt_prio   = BSP_USART_PREEMPT_PRIO,   /* 抢占优先级 */
        .irq_sub_prio       = BSP_USART_SUB_PRIO,       /* 响应优先级 */
    },
    [BSP_UART4] = {
    ...
    [BSP_UART5] = {
        ...
        .irq_it             = USART_IT_RXNE,            /* 判断是否在串口5的 RXNE 标志位上产生了中断 */
        .irq_preempt_prio   = BSP_USART_PREEMPT_PRIO,   /* 抢占优先级 */
        .irq_sub_prio       = BSP_USART_SUB_PRIO,       /* 响应优先级 */
    },
};
...
{
    /* 配置优先级分组：采用分组2（抢占优先级：0-3   响应优先级：0-3）*/
    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);
}

static void BSP_USARTIT_Config(bsp_usart_t id)
{
...
void BSP_USART_Init(bsp_usart_t id)
{
    ...
    /* 初始化串口中断优先级分组 */
    BSP_USARTIT_Config(id);
    /* 使能串口接收中断 */
    USART_ITConfig(hw->usartx, hw->irq_it, ENABLE);
...
#if 0
uint8_t BSP_USART_ReceiveData(bsp_usart_t id)
{
    if(id >= BSP_USART_MAX) return;
...
// void BSP_USART_Callback(USART_TypeDef *usart)
// {
//     uint8_t data = USART_ReceiveData(usart);
//     BSP_USART_SendByte(g_stdio_usart, data);
// }