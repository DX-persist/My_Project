...
bool BSP_ESP8266_ConnectTCP(void)
{
	char cmd[100];
    memset(cmd, '\0', sizeof(cmd));
	ESP8266_Status_t retval = ESP8266_ERROR;
	/* 连接TCP服务器 */
    ...
    BSP_ESP8266_ClearRxBuffer();
    BSP_ESP8266_SendCmd(cmd);
    retval = BSP_ESP8266_WaitForResponse("CONNECT", "ERROR", 5000);
	//printf("retval = %d(0=ok 1=error 2=timeout)\r\n",retval);
	if(retval == ESP8266_OK){
        return true;
	}else{
        return false;
	}
...
bool BSP_ESP8266_SetTransMode(void)
{
	...
	ESP8266_Status_t result;
	BSP_ESP8266_ClearRxBuffer();
	//printf("%s|%s|%d 开始发送透传指令\r\n",__FILE__,__func__,__LINE__);
    BSP_ESP8266_SendCmd(ESP8266_SET_TRAN_MODE_CMD);
	//printf("%s|%s|%d 透传指令发送成功,等待回应...\r\n",__FILE__,__func__,__LINE__);
    result = BSP_ESP8266_WaitForResponse(ESP8266_CMD_RESPONSE_SUCCESS, NULL, 2000);
    //printf("result = %d (0=OK, 1=ERROR 2=TIMEOUT)\r\n");
	if(result == ESP8266_TIMEOUT){
		return false;
	}

	/* 发送消息,收到">"表示此时双方可以互发消息了 */

	//printf("%s|%s|%d 开始发送发送消息指令\r\n",__FILE__,__func__,__LINE__);
	BSP_ESP8266_ClearRxBuffer();
    BSP_ESP8266_SendCmd(ESP8266_SEND_MSG_CMD);
	//printf("%s|%s|%d 发送消息指令发送成功,等待回应...\r\n",__FILE__,__func__,__LINE__);
    BSP_ESP8266_WaitForResponse(">", NULL, 2000);
    //printf("result = %d (0=OK, 1=ERROR 2=TIMEOUT)\r\n");
	if(result == ESP8266_OK){
		return true;
	}else{
...
void BSP_ESP8266_ReceiveData(char *msg_buffer, uint16_t max_len)
{
    int data;
    uint16_t i = 0;
    ...
    while(i < max_len - 1) {
        ...
        if(data >= 0) {
            msg_buffer[i++] = (char)data;
            start = BSP_GetTick();  // 收到数据，重置超时计时
