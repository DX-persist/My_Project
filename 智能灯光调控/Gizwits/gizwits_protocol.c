...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagchuanghu)
    {
        info->event[info->num] = EVENT_chuanghu;
        info->num++;
        dataPoints->valuechuanghu = gizStandardDecompressionValue(chuanghu_BYTEOFFSET,chuanghu_BITOFFSET,chuanghu_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagbaojing)
    {
        info->event[info->num] = EVENT_baojing;
        info->num++;
        dataPoints->valuebaojing = gizStandardDecompressionValue(baojing_BYTEOFFSET,baojing_BITOFFSET,baojing_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuechuanghu != cur->valuechuanghu)
    {
        GIZWITS_LOG("valuechuanghu Changed\n");
        ret = 1;
    }
    if(last->valuebaojing != cur->valuebaojing)
    {
        GIZWITS_LOG("valuebaojing Changed\n");
        ret = 1;
    }

    if(last->valueco != cur->valueco)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueco Changed\n");
            ret = 1;
        }
    }
    if(last->valueguang != cur->valueguang)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueguang Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(chuanghu_BYTEOFFSET,chuanghu_BITOFFSET,chuanghu_LEN,(uint8_t *)devStatusPtr,dataPoints->valuechuanghu);
    gizStandardCompressValue(baojing_BYTEOFFSET,baojing_BITOFFSET,baojing_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebaojing);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueco = gizY2X(co_RATIO,  co_ADDITION, dataPoints->valueco); 
    devStatusPtr->valueguang = gizY2X(guang_RATIO,  guang_ADDITION, dataPoints->valueguang); 
