 ...
 * @brief   基本定时器时基定时示例程序
 * @details
 * 本示例演示如何使用 STM32F10x 基本定时器 TIM7 作为基础时基定时器使用，
...
int main(void)
{
     ...
     * - 因此更新事件周期为 1 ms
     */
    bsp_basetimer_config_t base_config = {
        .prescaler = 72 - 1,
    ...
    /* 初始化 TIM7 基础时基与更新中断 */
    BSP_BaseTIM_Init(BSP_BASE_TIMER7, &base_config);
    ...
    while(1){
        /**
         * @note
         * 当前 `basetimer_cnt[BSP_BASE_TIMER7]` 每 1ms 自增一次。
